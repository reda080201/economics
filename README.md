...
백테스트와 캘리브레이션은 실제값을 그대로 복사하지 않고, 첫 관측값에서 시작해 공통 response function과 과거 외생 변수로 recursive simulated path를 만든 뒤 실제 데이터와 비교합니다. 백테스트 입력은 실시간 엔진과 같은 정규화 범위(신뢰 ±0.012, 가동률 ±0.008, 실질금리, 불확실성)를 사용해 계수 해석이 달라지지 않게 합니다. 캘리브레이션 결과는 기본 loss, 보정 후 loss, 개선률, 선택된 파라미터 배율, GDP/물가/실업률별 적합도를 함께 표시합니다.
...
GDP형 지출 집계는 `C + I + G + X - M`을 사용합니다. 수출은 소비로 중복 집계하지 않고 현재 tick의 수출 flow로 기록합니다. 수입은 중간재·에너지·소비재·자본재 flow로 분리하며, 수입가격지수 100에서도 기본 수입액이 존재합니다. 가격지수 변화는 명목 수입액과 별도 수입물가 압력을 함께 조정합니다. 스무딩된 무역수지는 별도 진단 지표로 유지됩니다.

## 재현성과 안정화 모드

시뮬레이션 초기화에는 기본 seed가 사용되며, 같은 seed와 같은 설정에서는 난수 기반 에이전트 생성·정책 이벤트·시장 변동이 같은 순서로 재현됩니다. snapshot 복원은 난수 상태도 함께 복원합니다. `npm run check`는 전체 JavaScript 구문을 검사하고, `npm test`는 response function 방향성, GDP·순수출, 한국 샘플 백테스트 포화 방지, 동일 seed 120 tick 재현성, 안정화 ON/OFF, 1,000 tick finite 실행을 확인합니다.
